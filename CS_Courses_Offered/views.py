...
def graph_data(request):
	...
	for row in class_offered:
		list.append({'Act_enrol':row.act_enrollment, 'SemCode':row.a_semester.term_code, 'Semester':row.a_semester.term_description, 'Class':row.a_course.course_title, })
...
def clickable_graph(request):
	...
	# return JsonResponse(context_instance=RequestContext(request), 'graph/graphs.html', data, safe=False)
	# return render_to_response('graph/graphs.html', semsters, context_instance=RequestContext(request))
	return render_to_response('graph/clickable.html', offered, context_instance=RequestContext(request))
